...
# register new user
@router.post("/user/register" , tags=["admin"])
async def register(card_id:str, disposition:str, full_name:str, password:str, token:str):
    if token_validator.validator(token, ('admin',)):
        try:
            ...
            sql_query_users = f"""
                        INSERT INTO users (card_id, name, password, group_name, token, token_create_time) 
                        VALUES ('{card_id}', '{full_name}', '{password}', NULL, NULL, NULL);
                        """
